...
public class BedrockBone {
    ...
    public void translateAndRotateAndScale(PoseStack poseStack) {
        ...
        // PJM: cache last() once instead of re-fetching per matrix op.
        PoseStack.Pose last = poseStack.last();
        last.pose().rotate(rotation);
        last.normal().rotate(rotation);
        // PJM: skip the two no-op matrix scales for the common identity (1,1,1) case;
        // keep the original all-zero skip so degenerate bones don't render zero-scale.
        if (this.xScale != 1.0F || this.yScale != 1.0F || this.zScale != 1.0F) {
            if (this.xScale != 0.0F || this.yScale != 0.0F || this.zScale != 0.0F) {
                last.pose().scale(this.xScale, this.yScale, this.zScale);
                last.normal().scale(this.xScale, this.yScale, this.zScale);
            }
        }
    }
    ...
    // PJM: reusable per-bone scratch — eliminates a Matrix4f allocation per call.
    // Recomputed every call (reads live local fields), so it stays correct even when
    // callers mutate this bone's x/y/z/rotation between calls (e.g. SBW's per-frame
    // bone overrides in transformCustomModelPart). The returned matrix is read-only;
    // mutating callers (getLocatorTransform, BedrockModelBase.getTransform) must copy.
    private final Matrix4f globalTransformScratch = new Matrix4f();

    public Matrix4f getGlobalTransform() {
        Matrix4f matrix = globalTransformScratch;
        matrix.identity();
