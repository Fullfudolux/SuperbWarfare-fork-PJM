...
//        }
    ...
    public void renderToBuffer(PoseStack poseStack, MultiBufferSource bufferSource, RenderType quadRenderType, RenderType triangleRenderType,
        ...
        // PJM: two-pass is required, not redundant — quad (QUADS) and triangle (TRIANGLES)
        // render types share Mojang's fallback BufferBuilder; getBuffer(triangle) after
        // getBuffer(quad) ENDS the quad buffer. So all cubes must be written before the
        // triangle buffer is opened. A single merged walk crashes with "Not building!".
    ...
    public Matrix4f getLocatorTransform(String locatorName) {
        ...
        // PJM: copy the (now reusable, read-only) global transform before mutating it
        // with translate/rotate below — getGlobalTransform() returns a shared scratch.
        Matrix4f transform = new Matrix4f(result.bone().getGlobalTransform());
